-- Prevent an administrator from changing their own allowlist entry.
-- The UI hides these controls too, but this check is authoritative at the DB boundary.
...
begin
  ...
  if v_email = v_actor_email then
    raise exception 'cannot manage own allowlist entry';
  end if;
