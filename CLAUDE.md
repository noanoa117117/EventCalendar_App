...
- Phase 1〜6（認証、空き時間登録、共有イベントカレンダー、イベント企画、3画面導線、管理画面）は実装済み。
- トップ `/` は共有イベントカレンダーの正規URL `/events` へリダイレクトする。独立画面は `/events`（①）、`/availability`（②）、`/planning`（③）。
- Supabase SQLは `0001` → … → `0008` の順に適用する。`0004` は管理ロールと管理用RPC、`0008` はGoogle Calendar同期用カラムを含む。
