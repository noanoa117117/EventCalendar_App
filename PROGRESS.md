...
- **モバイル空き時間UIの再修正（2026-08-08）**: 通常の `/availability` で使う横スクロール式プリセットレールを、パターン選択ボタンと独立した44pxの編集ボタンへ分離した。編集・削除は既存の `PresetDialog` を使い、所有者・`canEdit`・下書き中の制約を維持する。
- **モバイル週表示の方針変更（2026-08-08）**: 以前のモバイル3日グリッド案は撤回し、イベントカレンダーの週表示を正本として、空き時間画面も月曜始まりの縦型7日リストへ置き換えた。モバイルでは7日×48枠グリッドをレンダリングせず、各日付行に曜日・今日バッジ・選択中メンバーの色と、連続枠を集約した時間帯（終端`00:00`は`24:00`）または「空き時間未登録」を表示する。本人が編集可能な日付だけをボタンにし、日別の既存下書き操作を追加するボトムシートを開く。シート内だけで48枠を縦に描画し、タップは1枠、350msの長押し後の縦ドラッグは連続枠を変更する。8px超の通常スクロールは変更せず、`pointercancel`も下書きを作らない。保存・取消・失敗時の下書き保持は既存の週単位 `draftOps` と `set_availability_batch` をそのまま利用する。デスクトップの7日×48枠グリッドとマウスドラッグは維持する。**検証**: `npm test`（54件）、`npm run lint`、`npx tsc --noEmit`、`npm run build`、`npm run build:worker`、`git diff --check` が成功。Playwright/対応Chromiumがこの実行環境に無いため、360×800・375×812・390×844・430×932の実機/viewport目視、iOS Safari・Android Chromeでの長押しドラッグ、safe area、下部パターンバー、保存失敗時の表示は未確認。管理画面関連は今回変更せず pending のまま。
