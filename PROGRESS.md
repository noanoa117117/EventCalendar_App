...
- **モバイル空き時間UIの再修正（2026-08-08）**: 通常の `/availability` で使う横スクロール式プリセットレールを、パターン選択ボタンと独立した44pxの編集ボタンへ分離した。編集・削除は既存の `PresetDialog` を使い、所有者・`canEdit`・下書き中の制約を維持する。週グリッドはタッチ開始直後に選択を始めず、12pxの移動量と方向で判定する。横方向はブラウザの横スクロールへ渡して下書きを作らず、縦方向だけ時間帯選択を始め、タップは従来どおり30分枠を切替える。`pointercancel` は必ず破棄する。初期描画のコールバックを保持して週ドラッグが保存されない問題も、最新コールバックをrefで参照するよう修正した。**検証**: `npm test`（46件）、`npm run lint`、`npx tsc --noEmit`、`npm run build`、`npm run build:worker`、`git diff --check` が成功。Playwrightは依存関係・対応Chromiumがこの実行環境に無く、360/390/430pxの実機/viewport操作は未確認。iOS SafariとAndroid Chromeでは、横スワイプ、縦ドラッグ、タップ、`pointercancel`、長いパターン名・多数パターンを実機で確認する。
