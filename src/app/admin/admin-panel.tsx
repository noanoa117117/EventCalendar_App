    ...
    <ul className="divide-y rounded-lg border md:hidden" aria-label="許可されたメールアドレス">{entries.map((entry) => <li key={entry.id} className="flex items-center gap-2 px-3 py-2">
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          {superUser ? <><label htmlFor={`mobile-role-${entry.id}`} className="sr-only">{entry.email} の権限</label><select id={`mobile-role-${entry.id}`} value={entry.role} disabled={busy} onChange={(event) => { const next = event.target.value as AllowedEmailRole; mutate(() => client.rpc("manage_allowed_email", { p_email: entry.email, p_role: next, p_enabled: entry.is_enabled }), () => setEntries((old) => old.map((item) => item.id === entry.id ? { ...item, role: next } : item)), entry.is_enabled && next === "member" ? entry.email.toLowerCase() : undefined); }} className="rounded border bg-card px-1.5 py-0.5">{roles.map((value) => <option key={value} value={value}>{value}</option>)}</select></> : <span>{entry.role}</span>}
      ...
      {(superUser || entry.role === "member") && <div className="flex shrink-0 gap-1">
        <Button size="sm" variant="outline" className="h-7 px-2 text-xs" disabled={busy} onClick={() => mutate(() => client.rpc("manage_allowed_email", { p_email: entry.email, p_role: entry.role, p_enabled: !entry.is_enabled }), () => setEntries((old) => old.map((item) => item.id === entry.id ? { ...item, is_enabled: !item.is_enabled } : item)), entry.is_enabled ? entry.email.toLowerCase() : undefined)}>{entry.is_enabled ? "無効化" : "有効化"}</Button>
        <Button size="sm" variant="destructive" className="h-7 px-2 text-xs" disabled={busy} onClick={() => { if (window.confirm(`${entry.email} を削除しますか？`)) mutate(() => client.rpc("delete_allowed_email", { p_email: entry.email }), () => setEntries((old) => old.filter((item) => item.id !== entry.id)), entry.email.toLowerCase()); }}>削除</Button>
      </div>}
    </li>)}</ul>
  </main>;
}
