...
export function EventCalendar({ currentUser, initialEvents, initialParticipants, people, role, preview = false, compact = false }: { currentUser: Profile; initialEvents: Event[]; initialParticipants: Participant[]; people: Profile[]; role?: AllowedEmailRole | null; preview?: boolean; compact?: boolean }) {
  ...
  return <main className={`@container flex flex-col ${compact ? "min-h-0" : "h-dvh"} bg-muted/20`}>
    ...
    <div className="mx-auto grid min-h-0 w-full max-w-7xl flex-1 gap-4 p-4 @3xl:grid-cols-[1fr_320px] @3xl:p-8">
...
function EventDetail({ event, participants, names, currentUser, isSuperUser, onRespond, onEdit, onCancel, onDelete, onResync, preview }: { event: Event; participants: Participant[]; names: Map<string, Profile>; currentUser: Profile; isSuperUser: boolean; onRespond: (s: EventParticipantStatus) => void; onEdit: () => void; onCancel: () => void; onDelete: () => void; onResync: () => void; preview: boolean }) {
