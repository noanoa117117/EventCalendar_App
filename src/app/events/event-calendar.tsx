...
export function EventCalendar({ currentUser, initialEvents, initialParticipants, people, role, preview = false, compact = false }: { currentUser: Profile; initialEvents: Event[]; initialParticipants: Participant[]; people: Profile[]; role?: AllowedEmailRole | null; preview?: boolean; compact?: boolean }) {
  ...
  const days = view === "month"
    ? eachDayOfInterval({ start: startOfWeek(startOfMonth(cursor), { weekStartsOn: 0 }), end: endOfWeek(endOfMonth(cursor), { weekStartsOn: 0 }) })
    : eachDayOfInterval({ start: startOfWeek(cursor, { weekStartsOn: 0 }), end: endOfWeek(cursor, { weekStartsOn: 0 }) });
  ...
  const visibleEvents = events.filter((event) => event.is_personal ? showPersonal : showShared);
  ...
  async function respond(status: EventParticipantStatus) {
    if (!selected || selected.is_personal) return;
    if (preview) {
  ...
  async function saveEvent(form: FormData) {
    ...
    const isPersonal = editing?.is_personal ?? creatingPersonal;
    ...
    if (preview) {
      const data: Event = editing
        ? { ...editing, ...payload }
        : { id: crypto.randomUUID(), ...payload, is_personal: isPersonal, created_by: currentUser.id, status: "published", created_at: new Date().toISOString(), google_sync_status: null, google_sync_error: null, google_synced_at: null };
    ...
    const result = editing
      ...
      : await client.from("events").insert({ ...payload, is_personal: isPersonal, created_by: currentUser.id, status: "published" as EventStatus }).select().single();
    ...
    if (!result.data.is_personal) triggerSync(result.data.id);
  }

  async function deletePersonalEvent(event: Event) {
    if (!event.is_personal || !window.confirm("この個人イベントを削除しますか？")) return;
    if (!preview) {
      const { error } = await createClient().from("events").delete().eq("id", event.id).eq("is_personal", true);
      if (error) { toast.error("個人イベントの削除に失敗しました。"); return; }
    }
    setEvents((old) => old.filter((item) => item.id !== event.id));
    setPanel({ kind: "none" });
    toast.success(preview ? "個人イベントを削除しました（ローカル）。" : "個人イベントを削除しました。");
  }
  ...
  async function triggerSync(eventId: string) {
    if (preview || events.find((event) => event.id === eventId)?.is_personal) return;
    try {
  ...
  return <main className={`@container flex flex-col ${compact ? "min-h-0" : "h-dvh"}`}>
    ...
    <div className="mx-auto grid min-h-0 w-full max-w-[1400px] flex-1 gap-4 p-4 @3xl:grid-cols-[1fr_320px] @3xl:p-8">
      <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border bg-card shadow-sm">
        ...
        {!compact && <div className="flex flex-wrap items-center justify-between gap-2 border-b p-2"><div className="flex items-center gap-2 text-xs"><button aria-pressed={showShared} onClick={() => setShowShared((value) => !value)} className={`rounded-full border px-2.5 py-1 ${showShared ? "border-event/40 bg-event-soft text-event-foreground" : "text-muted-foreground"}`}>共有</button><button aria-pressed={showPersonal} onClick={() => setShowPersonal((value) => !value)} className={`rounded-full border px-2.5 py-1 ${showPersonal ? "border-personal/40 bg-personal-soft text-personal-foreground" : "text-muted-foreground"}`}>個人</button></div><div className="flex items-center gap-2 @3xl:hidden">{(role === "admin" || role === "super_user") && <Link href="/admin" className="flex items-center gap-1.5 rounded-full border border-warning/30 bg-warning-soft px-3 py-1.5 text-xs font-medium text-warning-foreground transition-colors hover:bg-warning/20"><Shield className="size-3.5" />管理</Link>}<Button size="sm" onClick={() => setCreationChoiceOpen(true)}><Plus className="size-4" /> イベント作成</Button></div></div>}
        {view === "week" ? (
          <div className="min-h-0 flex-1 divide-y overflow-y-auto @3xl:hidden">{days.map((day) => { const dayEvents = visibleEvents.filter((event) => isSameDay(asJstCalendarDate(event.start_at), day)); const today = isSameDay(day, asJstCalendarDate(new Date())); return <div key={day.toISOString()} className={`px-3 py-2 ${today ? "bg-today/10" : ""}`}><div className="mb-1 flex items-center gap-2"><span className="text-sm font-semibold">{format(day, "M/d", { locale: ja })}</span><span className="text-xs text-muted-foreground">{format(day, "EEEE", { locale: ja })}</span>{today && <span className="rounded-full bg-today px-1.5 py-0.5 text-[10px] font-medium text-primary-foreground">今日</span>}</div>{dayEvents.length > 0 ? dayEvents.map((event) => <button key={event.id} onClick={() => setPanel({ kind: "detail", event })} className={`mb-1 block w-full truncate rounded-md px-2 py-1.5 text-left text-sm hover:bg-event-soft ${event.status === "cancelled" ? "bg-muted text-muted-foreground line-through" : event.is_personal ? "bg-personal-soft text-personal-foreground" : "bg-event-soft text-event-foreground"}`}>{time(event.start_at)} {event.title}</button>) : <p className="text-xs text-muted-foreground">予定なし</p>}</div>; })}</div>
        ) : null}
        <div className={`grid grid-cols-7 border-b text-center text-xs font-medium text-muted-foreground ${view === "week" ? "hidden @3xl:grid" : ""}`}>{["日", "月", "火", "水", "木", "金", "土"].map((d) => <div key={d} className="py-2">{d}</div>)}</div>
        <div className={`grid flex-1 grid-cols-7 ${view === "week" ? "hidden @3xl:grid" : ""}`} style={{ gridTemplateRows: `repeat(${Math.ceil(days.length / 7)}, minmax(0, 1fr))` }}>{days.map((day) => { const dayEvents = visibleEvents.filter((event) => isSameDay(asJstCalendarDate(event.start_at), day)); const sharedCount = dayEvents.filter((event) => !event.is_personal).length; const personalCount = dayEvents.filter((event) => event.is_personal).length; return <div key={day.toISOString()} onClick={() => { if (dayEvents.length === 1) setPanel({ kind: "detail", event: dayEvents[0] }); else if (dayEvents.length > 1) setPanel({ kind: "day-list", date: day, events: dayEvents }); }} className={`border-b border-r p-1 @3xl:p-1.5 ${!isSameMonth(day, cursor) && view === "month" ? "bg-muted/30 text-muted-foreground" : ""} ${dayEvents.length >= 1 ? "cursor-pointer @3xl:cursor-default" : ""}`}><div className="mb-0.5 text-center text-xs @3xl:mb-1 @3xl:text-left">{format(day, "d")}</div>{dayEvents.length > 0 && <div className="flex justify-center gap-0.5 @3xl:hidden">{sharedCount > 0 && <span className="flex size-5 items-center justify-center rounded-full bg-event-soft text-[10px] font-medium text-event-foreground" aria-label={`共有イベント${sharedCount}件`}>{sharedCount}</span>}{personalCount > 0 && <span className="flex size-5 items-center justify-center rounded-full bg-personal-soft text-[10px] font-medium text-personal-foreground" aria-label={`個人イベント${personalCount}件`}>{personalCount}</span>}</div>}<div className="hidden @3xl:block">{dayEvents.map((event) => <button key={event.id} onClick={(e) => { e.stopPropagation(); setPanel({ kind: "detail", event }); }} className={`mb-1 block w-full truncate rounded-md px-1.5 py-1 text-left text-xs hover:bg-event-soft ${event.status === "cancelled" ? "bg-muted text-muted-foreground line-through" : event.is_personal ? "bg-personal-soft text-personal-foreground" : "bg-event-soft text-event-foreground"}`}>{time(event.start_at)} {event.title}</button>)}</div></div>; })}</div>
      </section>
      <aside className="hidden rounded-xl border bg-card p-4 shadow-sm @3xl:block">{panel.kind === "detail" ? <EventDetail event={panel.event} participants={selectedParticipants} names={names} currentUser={currentUser} onRespond={respond} onEdit={() => setPanel({ kind: "form", event: panel.event })} onCancel={() => cancelEvent(panel.event)} onDelete={() => deletePersonalEvent(panel.event)} onResync={() => triggerSync(panel.event.id)} preview={preview} /> : panel.kind === "form" ? <EventForm event={panel.event} isPersonal={panel.event?.is_personal ?? creatingPersonal} onSubmit={saveEvent} onClose={() => setPanel({ kind: "none" })} /> : <p className="text-sm text-muted-foreground">イベントを選択してください。</p>}</aside>
    </div>
    {panel.kind !== "none" && <div className="fixed inset-0 z-[60] bg-scrim @3xl:hidden" onClick={() => setPanel({ kind: "none" })}><section className="absolute inset-x-0 bottom-0 max-h-[85dvh] overflow-y-auto rounded-t-2xl border bg-card p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-xl" onClick={(event) => event.stopPropagation()}><div className="mb-2 flex justify-end"><Button variant="ghost" size="icon" aria-label="閉じる" onClick={() => setPanel({ kind: "none" })}><X /></Button></div>{panel.kind === "detail" ? <EventDetail event={panel.event} participants={selectedParticipants} names={names} currentUser={currentUser} onRespond={respond} onEdit={() => setPanel({ kind: "form", event: panel.event })} onCancel={() => cancelEvent(panel.event)} onDelete={() => deletePersonalEvent(panel.event)} onResync={() => triggerSync(panel.event.id)} preview={preview} /> : panel.kind === "day-list" ? <div className="space-y-2"><h2 className="text-lg font-semibold">{format(panel.date, "M月d日（EEEE）", { locale: ja })}</h2><div className="divide-y">{panel.events.map((event) => <button key={event.id} onClick={() => setPanel({ kind: "detail", event })} className={`block w-full px-2 py-2.5 text-left text-sm hover:bg-event-soft ${event.status === "cancelled" ? "text-muted-foreground line-through" : ""}`}><span className="font-medium">{time(event.start_at)}</span> {event.title}</button>)}</div></div> : <EventForm event={panel.event} isPersonal={panel.event?.is_personal ?? creatingPersonal} onSubmit={saveEvent} onClose={() => setPanel({ kind: "none" })} />}</section></div>}
    <Dialog open={creationChoiceOpen} onOpenChange={setCreationChoiceOpen}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>予定を作る前にメンバーの空き時間を確認しますか？</DialogTitle><DialogDescription>空き時間を確認して共通の日時を選ぶことも、確認せずに予定を作ることもできます。</DialogDescription></DialogHeader><div className="flex flex-col gap-2 pt-4"><Button className="w-full" variant="outline" onClick={() => router.push("/planning")}>空き時間を確認する</Button><Button className="w-full" onClick={() => { setCreationChoiceOpen(false); setCreatingPersonal(false); setPanel({ kind: "form", event: null }); }}>共有イベントを確認せずに作成</Button><Button className="w-full bg-personal text-white hover:bg-personal/90" onClick={() => { setCreationChoiceOpen(false); setCreatingPersonal(true); setPanel({ kind: "form", event: null }); }}>個人イベントを作成</Button></div></DialogContent></Dialog>
  </main>;
}

function EventDetail({ event, participants, names, currentUser, onRespond, onEdit, onCancel, onDelete, onResync, preview }: { event: Event; participants: Participant[]; names: Map<string, Profile>; currentUser: Profile; onRespond: (s: EventParticipantStatus) => void; onEdit: () => void; onCancel: () => void; onDelete: () => void; onResync: () => void; preview: boolean }) {
  ...
  return <div className="space-y-4"><div className="flex items-start justify-between gap-2"><div><p className="text-xs text-muted-foreground">{event.status === "cancelled" ? "キャンセル済み" : "イベント詳細"}</p><h2 className="text-xl font-semibold">{event.title}</h2><p className="text-sm text-muted-foreground">主催: {names.get(event.created_by)?.nickname ?? "メンバー"}</p></div>{event.created_by === currentUser.id && event.status !== "cancelled" && <div className="flex"><Button variant="ghost" size="icon" aria-label="編集" onClick={onEdit}><Pencil /></Button>{event.is_personal ? <Button variant="ghost" size="icon" aria-label="削除" onClick={onDelete}><Trash2 /></Button> : <Button variant="ghost" size="icon" aria-label="キャンセル" onClick={onCancel}><CircleX /></Button>}</div>}</div><p className="text-sm">{startDateLabel} {startTimeLabel}{sameDay ? `–${endTimeLabel}` : <><br />〜 {endDateLabel} {endTimeLabel}</>}</p>{event.description && <p className="whitespace-pre-wrap text-sm text-muted-foreground">{event.description}</p>}{!event.is_personal && event.status !== "cancelled" && <div className="space-y-2"><p className="flex items-center gap-1 text-sm font-medium"><Users className="size-4" />参加表明</p><div className="flex flex-wrap gap-2">{(Object.keys(statusLabel) as EventParticipantStatus[]).map((status) => <Button key={status} size="sm" variant={mine === status ? "default" : "outline"} onClick={() => onRespond(status)}>{statusLabel[status]}</Button>)}</div><p className="text-xs text-muted-foreground">参加表明済み {participants.length}名</p></div>}{!event.is_personal && <div className="space-y-3 border-t pt-3 text-sm">{grouped.map((group) => <div key={group.status}><p className="mb-1 font-medium">{statusLabel[group.status]} ({group.people.length}名)</p><div className="space-y-1">{group.people.map((participant) => <div key={participant.user_id} className="text-muted-foreground">{names.get(participant.user_id)?.nickname ?? "メンバー"}</div>)}</div></div>)}{!participants.length && <p className="text-muted-foreground">まだ参加表明はありません。</p>}</div>}{!preview && !event.is_personal && event.google_sync_status && <div className="border-t pt-3 text-xs">{event.google_sync_status === "synced" && <p className="text-muted-foreground">Googleカレンダーに同期済み</p>}{event.google_sync_status === "pending" && <p className="text-muted-foreground">同期中...</p>}{event.google_sync_status === "failed" && <div className="space-y-1"><p className="text-destructive">同期失敗{event.google_sync_error ? `: ${event.google_sync_error}` : ""}</p>{event.created_by === currentUser.id && <Button size="sm" variant="outline" onClick={onResync}>再同期</Button>}</div>}</div>}</div>;
}

function EventForm({ event, isPersonal, onSubmit, onClose }: { event: Event | null; isPersonal: boolean; onSubmit: (form: FormData) => Promise<void>; onClose: () => void }) {
